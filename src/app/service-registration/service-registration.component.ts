...
export class ServiceRegistrationComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    // Przygotuj dane do zapisania w enumeration
    const formValues = this.registrationForm.value;
    
    // Połącz wszystkie dane w jeden string z przejściami do nowej linii - format bardziej skondensowany
    ...
    // Wyślij dane do API - zapisz jako nową wartość w enumeration
    this.http.post('http://localhost:8080/api/enumerations/SERVICE_REGISTRATIONS', [serviceData])
      .subscribe({
        next: () => {
